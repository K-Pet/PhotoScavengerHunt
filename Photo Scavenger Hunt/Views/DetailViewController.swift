...
class DetailViewController: UIViewController {
    ...
    @IBOutlet weak var imageItemsLabel: UILabel!
    @IBOutlet weak var imageItemsDescriptionLabel: UILabel!
    ...
    /// Configure UI for the given location
    private func updateUI() {
        imageItemsLabel.text = imageItems.title
        imageItemsDescriptionLabel.text = imageItems.description

        let completedImage = UIImage(systemName: imageItems.isComplete ? "checkmark.circle.fill" : "circle")

        // calling `withRenderingMode(.alwaysTemplate)` on an image allows for coloring the image via it's `tintColor` property.
        checkDotImageView.image = completedImage?.withRenderingMode(.alwaysTemplate)
//
        let color: UIColor = imageItems.isComplete ? .systemBlue : .tertiaryLabel
        checkDotImageView.tintColor = color

        addPhotoButton.isHidden = imageItems.isComplete
    }
    
    func updateMapView() {
        // Make sure the location has image location.
        guard let imageLocation = imageItems.imageLocation else { return }
...
extension DetailViewController: PHPickerViewControllerDelegate {
    ...
    func presentGoToSettingsAlert() {
        let alertController = UIAlertController (
            ...
            message: "In order to post a photo to complete a imageItems, we need access to your photo library. You can allow access in Settings",
    ...
    func picker(_ picker: PHPickerViewController, didFinishPicking results: [PHPickerResult]) {
        ...
        guard let assetId = result?.assetIdentifier,
              let imageItems = PHAsset.fetchAssets(withLocalIdentifiers: [assetId], options: nil).firstObject?.location else {
            return
        }

        print("📍 Image location coordinate: \(imageItems.coordinate)")
        ...
        provider.loadObject(ofClass: UIImage.self) { [weak self] object, error in
            ...
            DispatchQueue.main.async { [weak self] in
                ...
                // Set the picked image and location on the location
                self?.imageItems.set(image, with: imageItems)

                // Update the UI since we've updated the location
                self?.updateUI()
...
extension DetailViewController: MKMapViewDelegate {
    ...
    func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
        ...
        guard let annotationView = mapView.dequeueReusableAnnotationView(withIdentifier: LocationAnnotationView.identifier, for: annotation) as? LocationAnnotationView else {
            fatalError("Unable to dequeue TaskAnnotationView")
        }

        // Configure the annotation view, passing in the task's image.
        annotationView.configure(with: imageItems.image)
